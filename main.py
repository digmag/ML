...
dfs = "train_cpy.csv test.csv".split()
...
for dframe in dfs:
  ...
  df['CryoSleep'] = df['CryoSleep'].fillna((df[['RoomService', 'FoodCourt', 'ShoppingMall', 'Spa', 'VRDeck']] != 0.0).any(axis=1))
  ...
  for c in cols:
    ...
    df[c] = df.apply(lambda row: 0 if row['CryoSleep'] else (mean_com if ~row['VIP'] else mean_VIP) if pd.isnull(row[c]) else row[c], axis=1)
...
target = "Transported"
y_train = dfs_edited[0][target]
X_train = dfs_edited[0].drop(columns=target).values

X_test = dfs_edited[1].drop(columns=target).values
y_test = dfs_edited[1][target]
